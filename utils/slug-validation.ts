...
const CONFIGS: Record<string, SlugValidationOptions> = {
  ...
  blogIndex: {
    ...
    customValidators: [(s) => (s !== '/blog' ? ["Blog index must be exactly '/blog'"] : [])],
  },
  homePage: {
    documentType: 'Home page',
    requireSlash: true,
    sanityDocumentType: 'homePage',
    customValidators: [(s) => (s !== '/' ? ["Home page must be exactly '/'"] : [])],
  },
  page: {
    documentType: 'Page',
    requireSlash: true,
    ...
    customValidators: [
      (slug) => {
        ...
        return reserved
          ...
          .map(([prefix, label]) => `Pages cannot use "${prefix}" prefix - reserved for ${label}`)
      },
    ],
  },
}
...
function validateDocTypeRules(slug: string, options: SlugValidationOptions): string[] {
  const errors: string[] = []
  ...
  switch (documentType) {
    ...
    case 'page': {
      if (currentSlug?.includes('/')) {
        ...
        if (segments.length > 1) return `/${segments.slice(0, -1).join('/')}/${clean}`
      }
      return `/${clean}`
    }
    default: {
